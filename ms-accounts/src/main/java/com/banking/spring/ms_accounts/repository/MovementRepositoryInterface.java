...
public interface MovementRepositoryInterface extends JpaRepository<Movement, Long> {
    ...
    @Query("SELECT m FROM Movement m JOIN FETCH m.account a WHERE m.deletedAt IS NULL AND a.deletedAt IS NULL ORDER BY m.date DESC")
    List<Movement> findAllActive();

    @Query("SELECT m FROM Movement m JOIN FETCH m.account a WHERE m.slug = :slug AND m.deletedAt IS NULL AND a.deletedAt IS NULL")
